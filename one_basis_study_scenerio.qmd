...
```{r, eval=FALSE}
#"test case", non-function version
log_reg_result <- glm(outcome ~ exposure + confounder_1_fixed_MI + confounder_2, 
                      data = missing_study,
                      family="binomial")
...
```{r}
#this worked? probably not the best implementation but I can't complain right now
CLQI_MI <- function(num_iterations = 10, missing_dataset, 
  ...
  for(i in 1:num_iterations) {
    ...
    #perform CLQI on missing dataset
    ...
    log_reg_result <- glm(outcome ~ exposure + confounder_1_CLQI + confounder_2, 
                          data = missing_dataset_temp,
                          family="binomial")

    log_odds_b1 <- log_reg_result$coefficients[2] #extracting the exposure variable coefficient
    odds_b1 <- exp(log_odds_b1)
  
    #then store this value in b1_imputations
    b1_imputations[i] <- odds_b1
  }

  return(b1_imputations)
  #now we spit out a dataframe with the iterated b1 coefficients
...
mean(CLQI_MI_results) - 1.1 #bias... super low
