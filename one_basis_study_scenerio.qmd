...
```{r}
uniform_values <- function(prop_missing) {
  u <- runif(1, min = 1, max = prop_missing) #aka from 0.01 to 0.99
...
```{r, eval=FALSE}
#non function version... there has to be a better way to do this, right???
#constant imputation example... replace the sqrt(2) with a function that performs the algorithm!
missing_study <- missing_study |>
  mutate(confounder_1_imputed = ifelse(is.na(confounder_1_missing), sqrt(2), confounder_1_missing))

#more testings required
u_vector <- uniform_values(prop_missing = prop_missing)
floor_quantile <- coefficient_data[u_vector[2], ] #floor
ceiling_quantile <- coefficient_data[u_vector[4], ] #ceiling

#now take a value with confounder_1 missing, just a test case
missing_study[16,]$predictor
missing_study[16,]$exposure
missing_study[16,]$outcome

#quantile regression results (with the log transform)... messy formula, there should be a better way
floor_quantile$b0
floor_quantile$b_predictor
floor_quantile$b_exposure
floor_quantile$b_outcome

#so this is terrible, but i don't know what else to do...
lower_quantile_value <- floor_quantile$b0 + (floor_quantile$b_predictor * missing_study[16,]$predictor) + (floor_quantile$b_exposure * missing_study[16,]$exposure) + (floor_quantile$b_outcome * missing_study[16,]$outcome)

upper_quantile_value <- ceiling_quantile$b0 + (ceiling_quantile$b_predictor * missing_study[16,]$predictor) + (ceiling_quantile$b_exposure * missing_study[16,]$exposure) + (ceiling_quantile$b_outcome * missing_study[16,]$outcome)

#imputation value
modulus <- u_vector[3]
imputation_value_transformed <- ((1-modulus)*lower_quantile_value) + (modulus*upper_quantile_value)

#lastly, we need to untransform this imputed value takes
min_imp <- 0 #just our case... will test cases later

missing_data_proportion = find_prop_missing(missing_data_study = missing_study, LD = "confounder_1_missing") / 100
LD_cutoff = as.numeric(quantile(missing_study$confounder_1, missing_data_proportion))
#then normal distribution to add random error... AHHHHHHH
max_imp <- LD_cutoff

imputed_value_regular <- inv_log_quant_transform(value = imputation_value_transformed, min = min_imp, max = max_imp)
#then we assign the missing value here
```

The function!

```{r}
#not entirely sure how i want to build this up
conditional_logistic_quantile_imputation <- function(basis_coefficients, study_for_imputation, 
                                                     var_for_imputation, prop_missing) {
  #random draw from a uniform dist
  u_vector <- uniform_values(prop_missing = prop_missing) 
  
  #choose the coefficients associated with the right tau value for quantile regression
  u_vector[2] #floor
  u_vector[3] #modulus
  u_vector[4] #ceiling
