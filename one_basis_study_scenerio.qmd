...
lower_quantile <- coefficient_data[u_vector[2]*100, ] #floor
upper_quantile <- coefficient_data[u_vector[4]*100, ] #ceiling

#now take a value with confounder_1 missing, just a test case
missing_study[16,]

#quantile regression results (with the log transform)
...
conditional_logistic_quantile_imputation <- function(basis_coefficients, study_for_imputation, var_for_imputation) {
  u_vector <- uniform_values() #random draw from a uniform dist
...
#current idea for the function:
#if the data point is missing:
#get my uniform values
#extract that uniform value row from basis_coefficients 
#extract the appropriate covariates from study_for_imputation
#do fitting
#spit out the value
#untransform the variable using min and max from study_for_imputation?
...
CLQI_MI <- function(num_iterations = 10) {
  #repeat this 10 times and save the desired parameters
