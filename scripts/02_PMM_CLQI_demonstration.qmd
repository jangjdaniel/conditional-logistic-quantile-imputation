...
logistic_quantile_imputation <- function(my_data, transformed_imputation_relationship, row_index) {
  ...
  b_intercept <-  reg_coeff |> filter(term == "(Intercept)") |> pull(estimate)
  b_outcome <-    reg_coeff |> filter(term == "outcome") |> pull(estimate)
  b_confounder <- reg_coeff |> filter(term == "confounder") |> pull(estimate)
  b_predictor <-  reg_coeff |> filter(term == "predictor") |> pull(estimate)
...
generated_data_for_CLQI <- generated_data_for_CLQI |>
  dplyr::select(biomarker_transformed, biomarker_MCAR_transformed, biomarker_MCAR_transformed_CLQI) |>
  mutate(diff = biomarker_MCAR_transformed_CLQI - biomarker_MCAR_transformed)
