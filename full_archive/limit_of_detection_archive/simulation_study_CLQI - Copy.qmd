...
Please read `readme` to understand the context behind this qmd. My hope is that this is easily readable. If not, please contact me: djang26@amherst.edu
...
### First, we will define necessary transformation functions we will use throughout this study. Of course there is `logit` and `expit`, as these are not explicitly built into R. We also have our `log_quant_transform` and `inv_log_quant_transform`, which are based off of Logistic Quantile Regression.
...
Make sure that the transformation functions work as wanted
...
###`coefficient_generator` generates the coefficients that will define the true relationships between the defined parameters
...
Another test case: these coefficients should be consistent
...
Make sure that the relationships we established are consistent
...
### `create_multiple_datasets` uses `create_data` to create multiple datasets. The distributions from which the data are generated for each dataset comes from information we get from `coefficient_generator`. For a common confounding mechanism, the `coefficient_generator` function is ignored
...
Test case for our data generation
...
data_generating_mechanism <- function(sample_size, LD, tau) {
  ...
  min = 0
  max = as.numeric(max(my_studies[[1]]$confounder_1)) #same as choosing the max
...
*This is an important test case, checking how - with our left censoring for the variable - the logistic regression behaves overall. This will take some time to run, but it will confirm that we have done eveerything right up to this point

```{r, eval=FALSE}

#we will be looking at three things: two log reg 
first_vect <- c()
second_vect <- c()
CC_vect <- c()
sqrt2_vect <- c()
...
for(j in 1:1000) {
  ...
  first_log <- glm(outcome ~ exposure + predictor + confounder_1, 
               data = as.data.frame(test_data[[1]]), 
               family = "binomial")

  second_log <- glm(outcome ~ exposure + predictor + confounder_1, 
               data = as.data.frame(test_data[[2]]), 
               family = "binomial")
  
  #now we extract the coefficients
  first_vect[j] <- summary(first_log)$coefficients[2]
  second_vect[j] <- summary(second_log)$coefficients[2]

  #####
  #what we will also look at is the complete case scenario
  second_miss_log <- glm(outcome ~ exposure + predictor + confounder_1_missing, 
                     data = as.data.frame(test_data[[2]]), 
                     family = "binomial")
  
  CC_vect[j] <- summary(second_miss_log)$coefficients[2]
  
  ####
  #now what if we impute sqrt(2) to all the missing values of the confounder??
  
  
  
}

#these histograms should be centered around 0.405 aka ln(1.5)
hist(one_vect)
hist(two_vect)
hist(CC_vect)
```
...
*warning: if you change the quantile regression, you need to change the data frame*

```{r}
logistic_quantile_regression_coefficients <- function(basis_study) {suppressWarnings({ 

 #suppress warnings, duh
  coefficient_data <- data.frame() #initialize empty data frame
    
  #take out the "inf"
  basis_study <- basis_study |>
    filter(confounder_1_transformed != "Inf")
    
  for(i in seq(from = 0.01, to = 0.99, by = 0.01)) {
    reg_coeff <- rq(confounder_1_transformed ~ exposure + outcome + predictor, 
                    data = basis_study, 
                    tau=i)
    ...
    new_data <- data.frame(
      ...
      b_outcome = reg_coeff$coefficients[3],
      b_predictor = reg_coeff$coefficients[4], #i tried removing this before, but added it back in
      quant = i)
...
```{r, eval=FALSE}
#a test case... why wasn't this here before?
test_data <- data_generating_mechanism(sample_size = 500, 
                                       LD = 0.60, 
                                       tau = 0)

logistic_quantile_regression_coefficients(as.data.frame(test_data[[1]]))
```
...
*warning: make sure to include all the values here for the lower and upper quantile values*
...
imputation_algorithm <- function(basis_coefficients, study_for_imputation, 
  ...
  lower_quantile_value <- floor_quantile$b0 +
    (floor_quantile$b_exposure * study_for_imputation[row_index,]$exposure) + (floor_quantile$b_outcome * study_for_imputation[row_index,]$outcome) + (floor_quantile$b_predictor * study_for_imputation[row_index,]$predictor)

  upper_quantile_value <- ceiling_quantile$b0 +
    (ceiling_quantile$b_exposure * study_for_imputation[row_index,]$exposure) + (ceiling_quantile$b_outcome * study_for_imputation[row_index,]$outcome) + (ceiling_quantile$b_predictor * study_for_imputation[row_index,]$predictor)
  ...
  min_imp <- 0
  max_imp <- as.numeric(max(study_for_imputation$confounder_1)) #to the max
...
exp(log_reg_result$coefficients[2]) - log(1.5) #bias
...
#rerunning this will give you different histograms based on the uniform value chosen
ggplot(data = missing_study_fixed_MI, aes(confounder_1_fixed_MI)) + 
  geom_histogram()
```
...
*THIS IS THE MAIN TEST CASE TO MAKE SURE CLQI IS WORKING*
...
#for ggplot uses only: transform the data so I can make the histogram look nice

LD_line <- min(CLQI_MI_one_iter$confounder_1_missing, na.rm = TRUE)

CLQI_MI_one_iter_ggplot <- CLQI_MI_one_iter |>
  dplyr::select(confounder_1, confounder_1_CLQI) |>
  pivot_longer(cols = c("confounder_1", "confounder_1_CLQI"),
               names_to = "confounder",
               values_to = "value")


ggplot(data = CLQI_MI_one_iter_ggplot, aes(x = value, group = confounder, color = confounder)) + 
  geom_density() +
  geom_vline(xintercept = LD_line)
```

`CLQI_MI` applies the CLQI algorithm to the missing_dataset 20 times and combines the results according to rubin's rules

```{r}
CLQI_MI <- function(num_iterations = 20, missing_dataset, 
                    basis_coefficients, var_for_imputation, #var_for_imputation is no longer used
                    specified_max) {
...
```{r, eval=FALSE}
#test case
#now with an estimate_and_variance... we will do manaul calculations to show everything is correct here
my_estimates <- data.frame(coefficients = c(1.1, 1.2, 1.11, 1.04, 1.6),
                           SE = c(0.04, 0.5, 0.22, 0.84, 0.11))

rubin_rules(my_estimates)
```
...
one_iteration <- function(specified_sample_size, specified_tau, specified_LD) {
  ...
  #regular logistic regression with the data just for a diagnostic. may become important
  regular_reg_result <- glm(outcome ~ exposure + predictor + confounder_1, 
                            data = missing_study, 
                            family = "binomial")
  
  regular_reg_result <- c(regular_reg_result$coefficients[2], summary(regular_reg_result)$coefficients[2,2])
...
###############################################################################################
  ...
  imputation_results_vector <- data.frame(estimate_CLQI = CLQI_MI_results[1],
                                          ...
                                          estimate_regular = regular_reg_result[1],
                                          SE_regular = regular_reg_result[2])
...
`repeat_full_simulation` is the function that runs `one_iteration` the specified number of times
...
```{r}
repeat_full_simulation <- function(specified_sample_size, specified_tau, specified_LD, sim_size, b1_true) {
  ...
  #after, we will calculate the statistics we desire... STILL HAVE NOT DONE COVERAGE YET
    sim_results_send <- sim_results |>
      mutate(bias_CLQI = estimate_CLQI - b1_true,
             ...
             bias_regular = estimate_regular - b1_true,
             MSE_regular = (bias_regular)^2 + (SE_regular)^2,
             coverage_regular = ifelse(estimate_regular - (t_star*SE_regular) <= b1_true & b1_true <= estimate_regular + (t_star*SE_regular), 1, 0)
             ) 
...
set.seed(605)
sim_setting_1_results <- repeat_full_simulation(specified_sample_size = 2500, 
                                                 specified_tau = 0, 
                                                 specified_LD = 0.50, #50% missing data
                                                 sim_size = 1000,
                                                 b1_true = log(1.5)) #LoTE dictates this is fine
...
mean(sim_setting_1_results$bias_fixed_MI)
mean(sim_setting_1_results$bias_constant)
mean(sim_setting_1_results$bias_CC)
mean(sim_setting_1_results$bias_regular)

#the MSE... hmm
mean(sim_setting_1_results$MSE_CLQI)
mean(sim_setting_1_results$MSE_fixed_MI)
mean(sim_setting_1_results$MSE_constant)
mean(sim_setting_1_results$MSE_CC)
mean(sim_setting_1_results$MSE_regular)

#and SE just in case
mean(sim_setting_1_results$SE_CLQI)
mean(sim_setting_1_results$SE_fixed_MI)
mean(sim_setting_1_results$SE_constant)
mean(sim_setting_1_results$SE_CC)
mean(sim_setting_1_results$SE_regular)
```
...
sim_setting_1_coverage <- sim_setting_1_results |>
  summarize(cov_perc_CLQI = sum(coverage_CLQI) / nrow(sim_setting_1_results), 
            ...
            cov_perc_regular = sum(coverage_regular) / nrow(sim_setting_1_results))

sim_setting_1_coverage
```

This is a strange diagnostic, but trust me on this

```{r}
sim_setting_1_proof <- sim_setting_1_results |>
  mutate(difference_CLQI_true = estimate_CLQI - estimate_regular,
         difference_fixed_MI_true = estimate_fixed_MI - estimate_regular,
         difference_CC_true = estimate_CC - estimate_regular,
         difference_constant_true = estimate_constant - estimate_regular) |>
  dplyr::select(difference_CLQI_true, difference_fixed_MI_true,
                difference_CC_true, difference_constant_true)


hist(sim_setting_1_proof$difference_CLQI_true)
hist(sim_setting_1_proof$difference_fixed_MI_true)
hist(sim_setting_1_proof$difference_CC_true)
hist(sim_setting_1_proof$difference_constant_true)
```
