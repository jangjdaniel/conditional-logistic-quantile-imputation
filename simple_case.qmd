...
#CLQI
  ...
  #now for the SE
  variance_within_MI <- mean((SE_CLQI)^2)
  variance_between_MI <- sum((values_CLQI - mean(values_CLQI))^2) / (length(values_CLQI) - 1)
  total_variance <- variance_within_MI + variance_between_MI + (variance_between_MI / length(values_CLQI))
  
  #now FOR COVERAGE
  t_star <- qt(0.975, df = my_sample - 1)
  coverage_CLQI[i] <- ifelse(mean(values_CLQI) - t_star * sqrt(total_variance) <= log(1.6) & log(1.6) <= mean(values_CLQI) + t_star * sqrt(total_variance), 1, 0)
...
```{r, eval=FALSE}
saveRDS(estimate_values_CLQI, file="./beta_results.rds")
```

```{r}
estimate_values_CLQI <- as.data.frame(readRDS(file="./beta_results.rds")) |>
