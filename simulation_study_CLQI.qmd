...
data_generating_mechanism <- function(sample_size, LD, tau) {
  ...
  min = 0
  max = ceiling(quantile(my_studies[[1]]$confounder_1, prob = 1)) #same as choosing the max
...
logistic_quantile_regression_coefficients <- function(basis_study) {
  ...
  for(i in seq(from = 0.01, to = 0.99, by = 0.01)) {
    reg_coeff <- rq(confounder_1_transformed ~ exposure + outcome + predictor, data = basis_study, tau=i)
...
imputation_algorithm <- function(basis_coefficients, study_for_imputation, 
  ...
  min_imp <- 0
  max_imp <- as.numeric(quantile(study_for_imputation$confounder_1, prob = 1)) #only go up to the 99th quantile for our data
