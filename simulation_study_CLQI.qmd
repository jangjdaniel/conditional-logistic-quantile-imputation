...
```{r, eval=FALSE}
#test case!
one_iteration(specified_sample_size = 500, specified_tau = 0.01)
```

Last but not least, `repetera_full_simulation` is the function that runs `one_iteration` the specified number of times (for readability sakes... everything should be a function)! This is Swedish for "repeat" (duh) to celebrate my time in Stockholm :)
...
```{r}
repetera_full_simulation <- function(specified_sample_size, specified_tau, sim_size, b1_true) {
  
  sim_results <- purrr::map_dfr(1:sim_size, ~ {
  
  # We go through one iteration of the simulation
  parameter_est <- one_iteration(specified_sample_size = specified_sample_size, specified_tau = specified_tau)
  ...
  #after, we will calculate the statistics we desire... STILL HAVE NOT DONE COVERAGE YET
    sim_results_send <- sim_results |>
      ...
      dplyr::select(bias_CLQI, MSE_CLQI, bias_fixed_MI, MSE_fixed_MI, 
                    bias_constant, MSE_constant, bias_CC, MSE_CC)
