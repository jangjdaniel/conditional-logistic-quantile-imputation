...
fixed_MI <- function(num_iterations = 10, missing_dataset, 
  ...
  for(i in 1:num_iterations) {
    ...
    b1_variance[i] <- summary(log_reg_result)$coefficients[2,2]
  }
...
constant_imputation <- function(missing_dataset, var_for_imputation, LD) {
  ...
  return(exp(log_reg_result$coefficients[2])) #for the exposure variable
}
```
...
complete_case_test - 1.1 #bias, should be pretty bad, because biomarker is MNAR
```
...
one_iteration <- function(specified_sample_size, specified_tau) {
  ...
  #our studies!
  basis_study <- as.data.frame(sim_data[[1]])
  missing_study <- as.data.frame(sim_data[[2]])
  the_LD <- min(missing_study$confounder_1_missing, na.rm=TRUE)
  ...
  #the CLQI_MI results
    CLQI_MI_results <- CLQI_MI(num_iterations = 10, 
                               missing_dataset = missing_study, #created from sim_data
                               basis_coefficients = basis_regression_coefficents, 
  ...
  imputation_results_vector <- c(CLQI_MI_results, fixed_MI_results, 
                                 constant_imputation_sqrt_2_results, complete_case_results)
...
Running the code 1000 times... note the runtime
...
#important things to initalize for the iteration
...
```{r}
tictoc::tic() #just for computation time checking

# Use purrr to iterate over simulations
sim_results <- purrr::map_dfr(1:sim_size, ~ {
  
  #we go through one iteration of the 
  parameter_est <- one_iteration(specified_sample_size = 500, specified_tau = 0.01)
  
  
  #calculate bias and MSE for each iteration (NOT SURE IF THIS IS CORRECT, DONT I DO THIS AFTER I FORGOT ****************)
  bias_val <- mean(parameter_est) - a_1_true
  MSE_val <- bias_val^2 + var(parameter_est) #is this correct?
